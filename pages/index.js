...
export default function Home() {
  ...
  const [items, setItems] = useState([]);        // base 明細（/results）
  const [famStats, setFamStats] = useState(null);// /stats/family
  const [rank, setRank] = useState(null);        // /rank
  const [leaderItems, setLeaderItems] = useState([]); // 榜首的明細（/results）
  const [next, setNext] = useState(null);
  const [loading, setLoading] = useState(false);
  const [err, setErr] = useState("");

  async function fetchResults(who, cursor = 0) {
    const url = `${api}/api/results?name=${encodeURIComponent(who)}&stroke=${encodeURIComponent(stroke)}&limit=200&cursor=${cursor}`;
  ...
  async function search(cursor = 0) {
    ...
    try {
      // 1) 目前人（明細）
      const j = await fetchResults(name, cursor);
      ...
      // 4) 榜首完整歷程（用 /results 抓「所有成績」，不是年度最佳）
      if (rj && rj.leader && rj.leader.name) {
        const lj = await fetchResults(rj.leader.name, 0);
        setLeaderItems(lj.items || []);
      } else {
        setLeaderItems([]);
      }
    } catch (e) {
  ...
  // ── 趨勢資料：把 base 與 leader 的所有成績合併到同一條「時間軸（毫秒）」 ───────────
  const trendData = useMemo(() => {
    const basePts = items
      .filter((x) => x.seconds > 0 && x["年份"])
      .map((x) => ({ year: x["年份"], ts: toTs(x["年份"]), base: x.seconds }))
      .sort((a, b) => a.ts - b.ts);

    const leaderPts = leaderItems
      .filter((x) => x.seconds > 0 && x["年份"])
      .map((x) => ({ year: x["年份"], ts: toTs(x["年份"]), lead: x.seconds }))
      .sort((a, b) => a.ts - b.ts);

    const byYearBase = Object.fromEntries(basePts.map((p) => [p.year, p.base]));
    const byYearLead = Object.fromEntries(leaderPts.map((p) => [p.year, p.lead]));
    const allYears = Array.from(new Set([...basePts.map(p => p.year), ...leaderPts.map(p => p.year)]));
    const merged = allYears.map((y) => ({ ts: toTs(y), year: y, base: byYearBase[y] ?? null, lead: byYearLead[y] ?? null }))
                           .sort((a, b) => a.ts - b.ts);

    // base 的 PB 點（用於 ReferenceDot）
    let pb = null;
    for (const m of merged) if (m.base && (!pb || m.base < pb.val)) pb = { ts: m.ts, val: m.base };
    return { merged, pb };
  }, [items, leaderItems]);

  // ── 排行卡片資料（前 10 + 若輸入選手不在前 10，補一行自己） ─────────────────────
  const rankRows = useMemo(() => {
    if (!rank || !rank.top) return [];
    const top10 = (rank.top || []).slice(0, 10);
    const inTop = top10.some((r) => r.name === name);
    if (inTop) return top10;
    if (rank.you) return [...top10, rank.you];
    return top10;
  }, [rank, name]);
      ...
      <div style={{ maxWidth: 1080, margin: "0 auto" }}>
        ...
        <Card>
          ...
          <div style={{ marginBottom: 8 }}>
            分母：{rank?.denominator ?? "-"}　你的名次：<b style={{ color:"#FFD36E" }}>{rank?.rank ?? "-"}</b>　百分位：{rank?.percentile!=null ? `${rank.percentile.toFixed(1)}%` : "-"}
          </div>
          <table style={table}>
            <thead>
            ...
            <tbody>
              {(rankRows||[]).map((r,i)=>(
                <tr key={i} style={r.name===name ? { background:"rgba(255,100,100,.08)"} : undefined}>
                  <td style={td}>{r.rank ?? "-"}</td>
                  <td style={{...td, fontWeight:r.name===name?800:600, color:r.name===name?"#FF7B7B":"#E9E9EC"}}>{r.name}</td>
        ...
        <Card>
          ...
          <div style={{ height: 360, marginTop: 8 }}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={trendData.merged} margin={{ top: 10, right: 16, bottom: 8, left: 0 }}>
                ...
                <Tooltip
                  ...
                  formatter={(v, key) => [fmtTime(v), key === "base" ? "成績" : "榜首"]}
                />
                <Line type="monotone" dataKey="base" stroke="#80A7FF" strokeWidth={2} dot={{ r: 3, stroke: "#0a0c10", strokeWidth: 1 }} activeDot={{ r: 5 }} />
                <Line type="monotone" dataKey="lead" stroke="#38D39F" strokeWidth={2} dot={false} />
                {trendData.pb && (
                  <ReferenceDot x={trendData.pb.ts} y={trendData.pb.val} r={5} fill="#FF6B6B" stroke="#0a0c10" strokeWidth={1}
                    isFront label={{ value:`PB ${fmtTime(trendData.pb.val)}`, position:"right", fill:"#FFC7C7", fontSize:12 }} />
                )}
              </LineChart>
            </ResponsiveContainer>
          </div>
            ...
            <tbody>
              {items.slice().sort((a,b)=> (b["年份"]||"")-(a["年份"]||"")).map((r,i)=>(
                <tr key={i}>
                  ...
                  <td style={{...td, color: (analysis.best!=null && r.seconds===analysis.best) ? "#FF7B7B" : "#E9E9EC", fontWeight:(analysis.best!=null && r.seconds===analysis.best)?800:600}}>
                    {fmtTime(r.seconds)}
                  </td>
                </tr>
              ))}
