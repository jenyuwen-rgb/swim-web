    ...
    <main style={{ minHeight: "100vh", background: "radial-gradient(1200px 600px at 20% -10%, #1f232b 0%, #0f1216 60%, #0a0c10 100%)", color: "#E9E9EC", padding: "24px 16px 80px" }}>
      <div style={{ maxWidth: 1080, margin: "0 auto" }}>
        ...
        <Card>
          ...
          <div style={{ marginBottom: 8, color: "#C8CDD7" }}>
            分母：{rank?.denom ?? "-"}　你的名次：<b style={{ color:"#ffd166" }}>{rank?.myRank ?? "-"}</b>　百分位：{rank?.percentile != null ? `${rank.percentile.toFixed(1)}%` : "-"}
          </div>
          ...
          <table style={table}>
            ...
            <tbody>
              {(rank?.top ?? []).map((r, i) => {
                ...
                return (
                  <tr key={`top-${i}`} style={isMe ? { background:"rgba(255,100,100,.07)" } : undefined}>
                    <td style={td}>{r.rank}</td>
                    <td style={{ ...td, color: isMe ? "#ff6b6b" : "#E9E9EC", fontWeight: isMe ? 800 : 600 }}>{r.name}</td>
              ...
              {/* 若本人不在前十，另外補一行 */}
              {(rank?.my && rank?.top && !rank.top.some(x => x.name === name)) && (
                <tr key="me-row" style={{ background:"rgba(255,100,100,.07)" }}>
                  <td style={td}>{rank.my.rank}</td>
                  <td style={{ ...td, color:"#ff6b6b", fontWeight:800 }}>{rank.my.name}</td>
                  <td style={td}>{fmtTime(rank.my.pb_seconds)}</td>
                  <td style={td}>{rank.my.year}</td>
                  <td style={td}>{rank.my.meet}</td>
                </tr>
              )}
            </tbody>
          </table>
          ...
          <div style={{ height: 340, marginTop: 8 }}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={trend.data} margin={{ top: 10, right: 16, bottom: 6, left: 0 }}>
                ...
                {trend.pb && (
                  <ReferenceDot
                    x={trend.pb.label} y={trend.pb.y} r={5}
                    fill="#FF6B6B" stroke="#0a0c10" strokeWidth={1} isFront
