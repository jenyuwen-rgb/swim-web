...
const GroupsTooltip = ({ active, label, payload, groupsChartKeys, groupsChartData, getBarColor }) => {
  ...
  const rows = payload
    .map((p) => {
      ...
      const barIdx = (groupsChartKeys || []).indexOf(dk);
      const rowIdx = (groupsChartData || []).findIndex((r) => r.group === row.group);
      const color = getBarColor(row, dk, rowIdx, barIdx);
      const meet = meta.meet || meta.event || meta.meet_name || meta.pb_meet || "";
      const year = meta.year || "";

      return { who, sec, year, meet, color };
    })
    .filter(Boolean);
  ...
  return (
    <div style={{ ...tooltipStyles.contentStyle, borderRadius: 8, minWidth: 220 }}>
      <div style={{ ...tooltipStyles.labelStyle, marginBottom: 6, fontSize: 12 }}>{label}</div>
      {rows.map((r, i) => (
        <div key={i} style={{ display: "flex", gap: 8, margin: "6px 0", alignItems: "center" }}>
          ...
          <div style={{ ...tooltipStyles.itemStyle, display: "flex", flexDirection: "column", gap: 2 }}>
            <strong style={{ color: "#fff" }}>{r.who}</strong>
            <span>{fmtTimeMMSS(r.sec)}｜{r.year || "—"}</span>
            {r.meet ? <span style={{ opacity: .85 }}>{r.meet}</span> : null}
          </div>
        </div>
      ))}
    </div>
                  ...
                  <BarChart data={groupsChartData} margin={{ top: 64, right: 10, bottom: 16, left: 10 }} isAnimationActive={false}>
                    ...
                    <Tooltip
                      ...
                      content={(tp) => (
                        <GroupsTooltip
                          {...tp}
                          groupsChartKeys={groupsChartKeys}
